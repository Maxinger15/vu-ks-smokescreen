	...
	Events:Subscribe(
		...
		function()
			...
			if InputManager:WentKeyDown(InputDeviceKeys.IDK_E) and pointOfAim.mode == FiringMode.Target then
				NetEvents:Send("vu-ks-smokescreen:Launch", pointOfAim.position)
...
function AreaStrike(position)
	for i = 1, STRIKE_MISSILE_COUNT do
		...
		secondStrike[#secondStrike + 1] = {position = position, points = {}, timer = fireAfter}
	end
end
...
Events:Subscribe(
	...
	function(dt)
		...
		for i = #arr, 1, -1 do
			...
			if arr[i].timer < 0 then
				arr[i].timer = MISSILE_AIRTIME
